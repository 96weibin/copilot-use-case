...
argument-hint: "Defect ID or link (optional). Skill will ask whether it's from old ALM or new ADO before proceeding."
user-invocable: true
---
...
```
User provides defect ID or link?
├── Yes → Ask: "这个 defect 是 ADO 还是旧 ALM (aspentech-alm)?"
│           ├── ADO  → Use ADO MCP tool (ado-wit_get_work_item)
│           │          Read Resolution/SCM fields from work item fields
│           └── ALM  → Use Chrome DevTools to open ALM page
│                      (same login-first flow as figma-design-inspector)
└── No  → Use currently open page in Chrome
            ↓
Login wall detected? (ALM path only)
...
### Step 1 — Resolve Target Defect & System

1. If argument or link is provided, **first ask the user**:
   - "这个 defect 是哪个系统的？"
   - Choices: `旧 ALM (aspentech-alm.visualstudio.com)` / `新 ADO (aspentech/AspenTech SAFe)`
2. Based on answer, branch:

**ALM path:**
- Use Chrome DevTools (`list_pages`) to find the ALM tab
- If no ALM tab → tell user to open `aspentech-alm.visualstudio.com` in Chrome and confirm when ready
- Navigate to the defect via search box or direct URL
- Proceed to Step 2 (login check) and Step 4 (SCM read via snapshot)

**ADO path:**
- Use `ado-wit_get_work_item` with the defect ID
- Look for SCM / changeset info in the work item fields or description
- Skip Chrome steps entirely; proceed directly to Step 5 (tf changeset)
